...
def affinity_loss(id_pred, im2obj, affinity_gt, max_rois, loss_type='bce', class_pred=None):
    """
    ...
    for i in range(valid_bs):
        ...
        sz_i = prod.shape[0]
        sz_j = prod.shape[1]
        affinity_pred[i, :sz_i, :sz_j] = prod
        
        # use class_pred to infer
        """
        class_view1 = class_pred[id_lower[i]:id_upper[i]]
        embed_view1 = F.one_hot(class_view1, num_classes=7).float()[:, 0]
        class_view2 = class_pred[id_lower[i + valid_bs]:id_upper[i + valid_bs]]
        embed_view2 = F.one_hot(class_view2, num_classes=7).float()[:, 0]
        prod = torch.mm(
            embed_view1, torch.t(embed_view2)
        )
        for j in range(prod.shape[0]):
            ids = prod[j].nonzero().cpu().numpy()
            if ids.shape[0] == 0:
                continue
            ids = ids[0]
            one_idx = np.random.choice(ids)
            prod[j] = 0
            prod[j][one_idx] = 1
    ...
    if loss_type == 'mse':
        loss_affinity = F.mse_loss(affinity_pred, affinity_gt, reduction='sum')
        loss_affinity /= valid_bs
        loss_affinity *= 0.1
    elif loss_type == 'bce':
        loss_affinity = F.binary_cross_entropy(affinity_pred, affinity_gt, reduction='sum')
        loss_affinity /= valid_bs
        loss_affinity *= 0.1
    elif loss_type == 'l2group':
        m = (affinity_gt == 1)
        loss_affinity = F.mse_loss(affinity_pred[m], affinity_gt[m], reduction='mean')
    elif loss_type == 'wmse':
        pos_m = (affinity_gt == 1)
        neg_m = (affinity_gt == 0)
        pos_l = F.mse_loss(affinity_pred[pos_m], affinity_gt[pos_m], reduction='mean')
        neg_l = F.mse_loss(affinity_pred[neg_m], affinity_gt[neg_m], reduction='mean')
        loss_affinity = pos_l + neg_l
    elif loss_type == 'focal':
        pt = affinity_pred
        target = affinity_gt
        pt = (1 - pt) * target + pt * (1 - target)
        w = pt.pow(2)
        loss_affinity = F.binary_cross_entropy(pt, target, w, reduction='mean')
    else:
