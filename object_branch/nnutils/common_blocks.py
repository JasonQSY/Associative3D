...
class ShapePredictor(nn.Module):
    ...
    def forward(self, feat):
        ...
        if self.pred_voxels:
            shape = torch.sigmoid(self.decoder.forward(shape))
        return shape
...
class IDPredictor(nn.Module):
    def __init__(self, nz_feat, n_dim=5):
        ...
        #self.pred_layer = nn.Linear(nz_feat, n_dim)
...
class CodePredictor(nn.Module):
    def __init__(
        ...
        self.id_predictor = None
        if pred_id:
            self.id_predictor = IDPredictor(nz_feat, nz_id)
    ...
    def forward(self, feat):
        ...
        if self.id_predictor is not None:
            id_pred = self.id_predictor(feat)
