"""
Script for Siamese dwr experiment.
"""
...
class SiameseDWRTrainer(train_utils.Trainer):
    def define_model(self):
        ...
        self.model = oc_net.OCNet(
            ...
            classify_rot=opts.classify_rot, nz_rot=opts.nz_rot, b_size=opts.batch_size * 2,
    ...
    def set_input(self, batch):
        ...
        common_src_classes = Variable(torch.cat(common_src_classes).cuda(non_blocking=True), requires_grad=False)
        common_trj_classes = Variable(torch.cat(common_trj_classes).cuda(non_blocking=True), requires_grad=False)
        ...
        self.relative_dir_mask = Variable(torch.cat(relative_dir_mask,dim=0).byte()).cuda(non_blocking=True)
        if opts.classify_dir and not opts.gmm_dir:
            ...
            self.relative_direction_rotation_unquantized = [Variable(t).cuda(non_blocking=True) for t in relative_direction_rotation]
            self.relative_direction_rotation = [Variable(t).cuda(non_blocking=True) for t in relative_direction_rotation_binned]
        else:
            self.relative_direction_rotation = [Variable(t).cuda(non_blocking=True) for t in relative_direction_rotation]
        ...
        self.codes_gt_quats = [
            Variable(t.cuda(non_blocking=True), requires_grad=False) for t in code_tensors_quats]
        codes_gt_keys = ['shape', 'scale', 'trans']
        self.codes_gt  ={key : Variable(code_tensors[key].cuda(non_blocking=True), requires_grad=False) 
                            for key in codes_gt_keys}
        self.codes_gt['quat'] = self.codes_gt_quats
       
        self.object_locations = [Variable(t_.cuda(non_blocking=True), requires_grad=False) for t_ in
                                 self.object_locations]
        self.object_scales = suncg_parse.batchify(code_tensors['scale'] + 1E-10, pos_rois[:,0])
        self.object_scales = [Variable(t_.cuda(non_blocking=True), requires_grad=False) for t_ in
                                 self.object_scales]
    ...
    def forward(self):
        ...
        loss_affinity = loss_utils.affinity_loss(id_pred, self.im2obj, self.affinity_gt, opts.max_rois, loss_type=opts.id_loss_type)
        ...
        for k in self.smoothed_factor_losses.keys():
            ...
            elif self.smoothed_factor_losses[k] == 0:
                self.smoothed_factor_losses[k] = self.loss_factors[k].item()
            else:
...
def main(_):
    ...
    trainer = SiameseDWRTrainer(FLAGS)
