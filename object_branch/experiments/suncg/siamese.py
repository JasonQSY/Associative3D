"""
Script for Siamese prediction experiment.
"""
...
class SiameseTrainer(train_utils.Trainer):
    ...
    def forward(self):
        ...
        self.total_loss, self.loss_factors = loss_utils.code_loss(
            ...
            quat_medoids = self.quat_medoids_var, 
            direction_medoids = self.direction_medoids_var,
        ...
        # calculate smoothed loss for visualization
        for k in self.smoothed_factor_losses.keys():
            if k not in self.loss_factors:
                continue
            if 'var' in k:
